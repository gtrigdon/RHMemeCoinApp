...
import abi from "./artifacts/build-info/RigdonhouseMemeCoin.json";

function App() {
  const [isWalletConnected, setIsWalletConnected] = useState(false);
  const [inputValue, setInputValue] = useState({ newOwnerAddress: "", walletAddress: "", transferAmount: "", burnAmount: "", mintAmount: "" });
  ...
  const contractAddress = '0xE9f84919ffAAD01E39eC4a20b3B82DA43Ae91EFA';
  ...
  const ownershipXFer = async (event) => {
    event.preventDefault();
    try {
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        ...
        const txn = await tokenContract.transferOwnership(inputValue.newOwnerAddress);
        console.log("Transferring Ownership...");
        await txn.wait();
        console.log("Ownership transferred..., txn.hash)");
  ...
  return (
    <main className="main-container">
      <h2 className="headline">
        <span className="headline-gradient">Rigdonhouse Meme Coin</span>
        ...
        <div className="mt-10 mb-10">
              <form className="form-style">
                <input
                  ...
                  value={inputValue.newOwnerAddress}
                />
                <button
                  className="btn-purple"
                  onClick={ownershipXFer}>
                  Ownership Transfers
                </button>
              </form>
            </div>
